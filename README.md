...
- `arxiv-email-daily.xml`：只汇总当天 QQ 邮箱 arXiv 邮件的精选结果，并显示邮件数、DeepSeek 完成数和各相关度区间统计。
- `arxiv-email-archive/`：邮箱专属日报的 JSON 和 HTML 历史快照。
- `arxiv-email-analysis/`：保存当天全部邮箱论文的 DeepSeek 相关度、中文标题、摘要、理由和研究启发；这些记录不会全部进入 Zotero。

邮箱专属管线使用独立的 `research-data/arxiv-email-state.json`，不会与融合速递共享邮件去重状态。它按 QQ IMAP 实际收件日期归档，由 DeepSeek 分析邮件内全部论文，再向 RSS 推送相关度不低于 45 分的前 30 篇。模型请求按每批 10 篇、最多 8 批并行执行；失败批次不会把邮件标记为完成，下次运行会自动重试。除每天 06:20 的常规更新外，工作日北京时间 20:30 会再运行一次邮箱专属管线，以等待当天 arXiv 邮件到达。
...
https://fengziclassmate.github.io/journal-rss/jgsa.xml
...
https://fengziclassmate.github.io/journal-rss/buildings.xml
https://fengziclassmate.github.io/journal-rss/buildings-current-issue.xml
https://fengziclassmate.github.io/journal-rss/sustainability.xml
https://fengziclassmate.github.io/journal-rss/sustainability-current-issue.xml
```
